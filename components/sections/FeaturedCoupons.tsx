...
const fallbackCards = [
  {
    ...
    logoHeight: 40,
    logoAlt: 'Myprotein',
    title: 'CouponsScript Exclusive! Extra 10% Off All Creatine',
    cta: 'Reveal Code',
    image:
      'https://www.retailmenot.com/imagery/homepage-feature-cards/01kkex4xvnz94nwsfgahh41h4q-image.fit_limit.quality_80.size_160x170.v1773248083.webp',
    code: 'CREATINE10',
    storeUrl: '#',
    storeName: 'Myprotein',
    discount: '10% Off',
  },
  {
    ...
    logoWidth: 72,
    logoHeight: 40,
    logoAlt: 'Ace Hardware',
    title: '5% Cash Back for Purchases Sitewide',
    cta: 'Shop Now',
    image:
      'https://www.retailmenot.com/imagery/homepage-feature-cards/01kkex6xm8db06hgwa77vaeyd0-image.fit_limit.quality_80.size_160x170.v1773248149.webp',
    code: '',
    storeUrl: '#',
  ...
  },
  {
    ...
    theme: 'purple',
    logo: null,
    logoAlt: 'CouponsScript',
    title: 'Guaranteed Cash Back from 4,000+ Brands',
    cta: 'Shop CouponsScript',
    image:
      'https://www.retailmenot.com/imagery/homepage-feature-cards/01kk9s71sgrffp151xmrahh9fj-image.fit_limit.quality_80.size_160x170.v1773076186.png.webp',
    code: '',
    storeUrl: '/cashback',
    storeName: 'CouponsScript',
    discount: '',
  },
];
...
function DefaultLogo({ name, isPurple }: { name?: string; isPurple?: boolean }) {
  return (
    <div
      className={`mt-3 w-10 h-10 rounded-lg flex items-center justify-center text-xs font-bold ${isPurple ? 'bg-primary/20 text-primary' : 'bg-gray-100 text-gray-500'}`}
    >
  ...
  return (
    ...
    >
      <div className="flex flex-col pl-5 pr-4 justify-start overflow-hidden">
        ...
        <p
          className={`my-3 text-sm font-semibold leading-tight line-clamp-3 ${isPurple ? 'text-primary' : 'text-gray-900 dark:text-gray-100'}`}
        >
          {card.title}
        </p>
        <p
          className={`text-xs uppercase tracking-widest underline underline-offset-4 font-bold ${isPurple ? 'text-primary' : 'text-gray-800 dark:text-gray-200'}`}
        >
...
function mapCouponToCard(coupon: any) {
  ...
  const logo = coupon.customLogo
    ? coupon.customLogo.startsWith('http')
      ? coupon.customLogo
      : `${serverUrl}${coupon.customLogo}`
    : store.logo
      ? store.logo.startsWith('http')
        ? store.logo
        : `${serverUrl}${store.logo}`
      : '';
