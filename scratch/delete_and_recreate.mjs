...
const actList = [
  "Flight Coupons", "Travel Coupons", "Hotel Coupons", "Bus Coupons",
  "Bike Rentals Coupons", "Utility Bill Payments Coupons", "Recharge Coupons",
  "Hosting Coupons", "Food & Lifestyle", "Pizza Coupons", "Meat & Dairy Coupons",
  "Groceries Coupons", "Fashion Coupons", "Footwear Coupons", "Jewellery Coupons",
  "Kids & Lifestyle Coupons", "Lingerie Coupons", "Gifts & Flowers Coupons",
  "Health & Personal Care", "Beauty Coupons", "Medicines Coupons",
  "Lab Tests Coupons", "Protein Supplements Coupons", "Services Coupons",
  "Eyewear Coupons", "Education Coupons", "Tech & Entertainment",
  "Electronics Coupons", "Kitchen Appliances Coupons", "Entertainment Coupons",
  "OTT Coupons", "Furniture Coupons"
];
...
async function main() {
  try {
    ...
    // 1. Fetch current categories and delete ones that contain " Coupons" or were just created
    const catRes = await fetch(`${API_URL}/public/categories/list`);
    let catData = await catRes.json();
    let existing = catData.data || catData || [];

    const toDelete = existing.filter(c => c.name.endsWith(' Coupons') || c.name.endsWith(' Coupon'));
    
    if (toDelete.length > 0) {
      console.log(`Deleting ${toDelete.length} wrong categories...`);
      const ids = toDelete.map(c => c._id);
      await fetch(`${API_URL}/admin/categories/bulk-delete`, {
         method: 'POST',
         headers: { 
      ...
      console.log('✓ Deleted wrong categories.');
    }

    // 2. Re-create correctly
    let categories = actList.map(item => item.replace(/\s+Coupons?/gi, '').trim());

    console.log(`Creating ${categories.length} correct categories...`);
